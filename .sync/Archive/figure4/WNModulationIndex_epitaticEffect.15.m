...
%% plot examples
for cc = 1:length(cell_number)
    c =  celll_number(cc);
    figure; subplot(3,1,1); hold on
    plot(nanmean(data(c).mNON_off), 'k')
    plot(nanmean(data(c).mNON_on), 'r--')
    title('running effect'); legend('sitting', 'running')
    subplot(3,1,2); hold on
    plot(nanmean(data(c).mNON_off), 'k')
    plot(nanmean(data1(c).mNON_off), 'c')
    title('running effect'); legend('laser off', 'laser on')
    subplot(3,1,3); hold on
    plot(nanmean(data(c).mNON_off), 'k')
    plot(nanmean(data1(c).mNON_on), 'c--')
    title('running effect'); legend('sitting + laser off', 'runnning +laser on')
    predicted_trace = nanmean(nanmean(data(c).mNON_on) +nanmean(data1(c).mNON_off));

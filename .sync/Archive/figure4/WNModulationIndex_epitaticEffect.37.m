...
for cc = 1:length(cell_number1)
    ...
    if ~isnan(c)
        st = data(c).mST_off;
        [x, sitting_st] = GaussSmooth(st, 20, [-300 900]);
        st = data(c).mST_on;
        [x, running_st] = GaussSmooth(st, 20, [-300 900]);
        st = data1(c).mST_off;
        [x, sitting_st_laser_on] = GaussSmooth(st, 20, [-300 900]);
        st = data1(c).mST_on;
        [x, running_st_laser_on] = GaussSmooth(st, 20, [-300 900]);

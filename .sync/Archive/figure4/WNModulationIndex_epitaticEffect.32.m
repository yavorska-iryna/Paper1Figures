...
for cc = 1:length(cell_number1)
    ...
    if ~isnan(c)
        st = data(c).mST_off;
        sitting_st = GaussSmooth(st*1000, 50, [-300 900]);
        st = data(c).mST_on;
        running_st = GaussSmooth(st*1000, 50, [-300 900]);
        st = data1(c).mST_off;
        sitting_st_laser_on = GaussSmooth(st*1000, 50, [-300 900]);
        st = data1(c).mST_on;
        running_st_laser_on = GaussSmooth(st*1000, 50, [-300 900]);
        figure; hold on; plot(sitting_st,'k')
        plot(running_st, 'k--')
        plot(sitting_st_laser_on, 'c')
        plot(running_st_laser_on, 'c--')
    end
end

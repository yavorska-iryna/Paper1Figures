...
for d = 10:10; %length(DIRS)
    if ~isempty(DIRS{d})
        ...
        [~, running_trace] = GaussSmooth(running_trace, 50, [0 length(running_trace)-1]);
        ...
        laser_start = stimlog(1).LaserStart;
        laser_width = stimlog(1).LaserWidth;
        for i = 1:length(Events)
             start = Events(i). soundcard_trigger_timestamp_sec*1000;

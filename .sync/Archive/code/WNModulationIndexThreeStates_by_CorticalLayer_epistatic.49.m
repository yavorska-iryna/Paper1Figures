...
for cc =1:length(data)
    if data(cc).dir<46
        ...
        if meanSpikeCount > 2 && CellsQualityStats.SNR(cc)>.5 && CellsQualityStats.uQ(cc)>10 
            ...
            [rs, h, stats] = ranksum( nanmean(ON(:)), nanmean(Spont(:)));
            evoked(cc,1) = h;
            zstats(cc,1) = stats.zval;
            
            Sust = [data(cc).mNSustained_on; data(cc).mNSustained_off; data(cc).pNSustained_on]; 
            [rs, h, stats] = ranksum( nanmean(Sust(:)), nanmean(Spont(:)));
            evoked(cc,2) = h;
            zstats(cc,2) = stats.zval;
            
            OFF = [data(cc).mNOFF_on; data(cc).mNOFF_off; data(cc).pNOFF_on]; 
            [rs, h, stats] = ranksum(nanmean(OFF(:)), nanmean(Spont(:)));
            evoked(cc,3) = h;
            zstats(cc,3) = stats.zval;
            
            [rs, h, stats] = ranksum( nanmean(data(cc).WNresponse), nanmean(data(cc).SSresponse)); %WN response is 0-600ms window
            evoked(cc,4) = h;

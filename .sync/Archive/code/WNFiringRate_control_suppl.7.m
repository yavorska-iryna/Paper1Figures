...
plot(WN_rs, WN_rsL, 'k.')
plot(WN_fs, WN_fsL, 'b.')
[rho,p]=corr([WN_rs; WN_fs], [WN_rsL; WN_fsL], 'Type', 'Spearman', 'rows', 'pairwise');
title(sprintf('Evoked, rho=%.4f,p=%.4f ', rho_rs, p_rs, rho_fs, p_fs))
xlabel('run - VIP'); ylabel('VIP - run'); lsline
subplot(2,1,2);hold on
plot(SP_rs, SP_rsL, 'k.')
plot(SP_fs, SP_fsL, 'b.')
xlabel('run - VIP'); ylabel('VIP - run'); lsline
[rho,p]=corr([SP_rs; SP_fs], [SP_rsL; SP_fsL], 'Type', 'Spearman', 'rows', 'pairwise');
title(sprintf('Spont, rs rho=%.4f,p=%.4f fs rho=%.4f,p=%.4f', rho, p))
...
[rho,p]=corr([WN_rs_plus; WN_fs_plus], [WN_exp_rs; WN_exp_fs], 'Type', 'Spearman', 'rows', 'pairwise');
title(sprintf('Evoked, rho=%.4f,p=%.4f ', rho, p))
subplot(2,1,2);hold on
plot(SP_rs_plus, SP_exp_rs, 'k.'); 
plot(SP_fs_plus, SP_exp_fs, 'b.');
xlabel('VIP + run'); ylabel('Expected'); lsline
[rho, p]=corr([SP_fs_plus; SP_rs_plus],[ SP_exp_fs; SP_exp_rs], 'Type', 'Spearman', 'rows', 'pairwise');

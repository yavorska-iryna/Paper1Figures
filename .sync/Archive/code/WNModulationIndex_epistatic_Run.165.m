...
figure; 
h1 = histogram(nanmean([WN1L(:,1)'; WN1L(:,2)']));
h1.BinWidth = 3;
hold on;
h2 = histogram(nanmean([WN1(:,1)'; WN1(:,2)']));
h2.BinWidth = 3;
ylabel('Number of cells')
xlabel('Firing Rate (Hz)')
legend('laser on', 'laser off')
[p,h,stats] = signrank(nanmean([WN1L(:,1)'; WN1L(:,2)']), nanmean([WN1(:,1)'; WN1(:,2)']))
title_str = sprintf('Evoked Activity, p = %d', p)
title(title_str)

figure; 
h1 = histogram(nanmean([SP1L(:,1)'; SP1L(:,2)']));
h1.BinWidth = .5;
hold on;
h2 = histogram(nanmean([SP1(:,1)'; SP1(:,2)']));
h2.BinWidth = .5;
ylabel('Number of cells')
xlabel('Firing Rate (Hz)')
legend('laser on', 'laser off')
[p,h,stats] = signrank(nanmean([SP1L(:,1)';SP1L(:,2)']), nanmean([SP1(:,1)'; SP1(:,2)']))
title_str = sprintf('Spontaneous Activity, p = %d', p)
title(title_str)

meanMI_sound_laser = nanmean(nanmean(WN1L(:),2) - nanmean(SP1L(:),2)./nanmean(WN1L(:),2) + nanmean(SP1L(:),2));
semMI_sound_laser = sem(nanmean(WN1L(:),2) - nanmean(SP1L(:),2)./nanmean(WN1L(:),2) + nanmean(SP1L(:),2));

meanMI_sound = nanmean(nanmean(WN1(:),2) - nanmean(SP1(:),2)./nanmean(WN1(:),2) + nanmean(SP1(:),2));
semMI_sound = sem(nanmean(WN1(:),2) - nanmean(SP1(:),2)./nanmean(WN1(:),2) + nanmean(SP1(:),2));

figure;
errorbar([1 2], [meanMI_sound_laser meanMI_sound], [semMI_sound_laser semMI_sound], 'ko-' )

...
for cc =1:length(data)
    if data(cc).dir < 30
        ...
        if meanSpikeCount > 3 && CellsQualityStats.SNR(cc)>.5 && CellsQualityStats.uQ(cc)>10
            ...
            %                 rs=0;
            %             end
            if h ==1 && stats.tstat > 0 && (nanmean(nanmean(data(cc).mNON_off)) - nanmean(nanmean(data(cc).mSSoff))) < 0
                figure; subplot(4,2,1)
                plot(data(cc).WNresponse);
                subplot(4,2,2); plot(data(cc).mNON_off)
                subplot(4,2,3); plot(data(cc).mSSoff)
                subplot(4,2,4); plot(data(cc).mNON_on)

...
x = nanmean(WN1L(:),2) - nanmean(SP1L(:),2)./nanmean(WN1L(:),2) + nanmean(SP1L(:),2);
indx = find(x ==-Inf); x(indx) = NaN;
meanMI_sound_laser = nanmean( nanmean([MI_sound_run_laser MI_sound_sit_laser],2));
semMI_sound_laser = nansem(x);

x = nanmean(WN1(:),2) - nanmean(SP1(:),2)./nanmean(WN1(:),2) + nanmean(SP1(:),2);
indx = find(x ==-Inf); x(indx) = NaN;
meanMI_sound = nanmean(x);
semMI_sound = nansem(x);

figure;hold on;
errorbar(1, meanMI_sound_laser, semMI_sound_laser , 'ko-' );
errorbar(2, meanMI_sound, semMI_sound , 'ko-' )

        ...
        if meanSpikeCount > 2 && CellsQualityStats.SNR(cc)>.5 && CellsQualityStats.uQ(cc)>10 
            ...
            if data(cc).dir~= 0 
                if data(cc).nrepsWNPon > nreps_check_pupil && data(cc).nrepsWNPoff > nreps_check_pupil %&& data1(cc).nrepsWNPon > nreps_check_pupil && data1(cc).nrepsWNPoff > nreps_check_pupil
                    ...
                    meanON(cc,2)= nanmean(nanmean(data(cc).pNON_on))-nanmean(nanmean(data(cc).mNson));%nanmean([nanmean(nanmean(data(cc).pNON_on)) nanmean(nanmean(data(cc).pNSustained_on)) nanmean(nanmean(data(cc).pNOFF_on))]);
                    meanON(cc,3)= nanmean(nanmean(data(cc).pNON_off))-nanmean(nanmean(data(cc).mNsoff));
                end
            end
            ...
            if data(cc).dir~= 0 
                if data1(cc).nrepsWNPon > nreps_check_pupil && data1(cc).nrepsWNPoff > nreps_check_pupil && data(cc).nrepsWNPon > nreps_check_pupil && data(cc).nrepsWNPoff > nreps_check_pupil
                    ...
                    meanONL(cc,2) =nanmean([nanmean(nanmean(data1(cc).pNON_on))])-nanmean(nanmean(data1(cc).mNson));;
                    meanONL(cc,3) =nanmean([nanmean(nanmean(data1(cc).pNON_off))])-nanmean(nanmean(data1(cc).mNsoff));;
                end
            end

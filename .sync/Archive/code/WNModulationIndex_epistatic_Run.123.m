...
legend('sitting - laser off', 'running - laser off', 'sitting - laser on', 'running - laser on')
xlabel('Modulation Index - Sound')
ylabel('Number of cells')
title(' all On responses')


indx = find(MI_sound_run<-.9);
MI_sound_runr(indx)= NaN;

fprintf('running vs sitting, laser off')
[p,h stats] =ranksum(MI_sound_run, MI_sound_sit)

indx = find(MI_sound_run_laser<-.9);
...
SNR_sound_run = WN1(:,1)./SP1(:,1);
SNR_sound_sit = WN1(:,2)./SP1(:,2);
[h,x] = hist(SNR_sound_sit, [0:.01:1]);
figure; hold on
plot(x,h, '-');
[h,x] = hist(SNR_sound_run, [0:.01:1]);
plot(x,h, '-');
legend('sitting', 'running')

MI_sound_runL = (WN1L(rs1,1) - SP1L(rs1,1))./ (WN1L(rs1,1) +SP1L(rs1,1));
MI_sound_sitL = (WN1L(rs1,2) - SP1L(rs1,2))./ (WN1L(rs1,2) +SP1L(rs1,2));
[h,x] = hist(MI_sound_sitL, [-1:.2:1]);
figure; hold on
plot(x,h);
[h,x] = hist(MI_sound_runL, [-1:.2:1]);
plot(x,h);
legend('sitting', 'running')
title('laser on')

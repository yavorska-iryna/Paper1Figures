% compute modulation index WN responses
...
nreps_check_running = 4; %number of repetitions in each condition for comparison
nreps_check_pupil = 8;
...
for cc =1:length(data)
    if data(cc).dir<30
        ...
        if meanSpikeCount > 2 & cells_stats(1,cc)>.5 && cells_stats(2,cc)>10 %& cells_stats(1,cc)<.89
...
evoked2 = evoked(:,4);% & zstats(:,4)>0;% & evoked(:,1)| evoked(:,4) | zstats(:,3)>1 & evoked(:,3);%evoked(:,1) | evoked(:,2) | evoked(:,3);logical(ones(length(evoked),1));
% indx = find(zstats(:,1) >0 & evoked(:,1) ==1);
% evoked1(indx,1) = 1;
% indx = find(zstats(:,2) >0 & evoked(:,2) ==1);
% evoked1(indx,2) = 1;
% indx = find(zstats(:,3) >0 & evoked(:,3) ==1);
% evoked1(indx,3) = 1;
% %evoked
% evoked2 = sum(evoked1,2);
% evoked2(evoked2>0 ) = 1; evoked2 = logical(evoked2);
%evoked
% evoked2 = sum(evoked1,2);
% evoked2(evoked2>0 ) = 1; evoked2 = logical(evoked2);
...
%spont
% Rs = logical(Rs); fs = logical(fs);
% modulation_indx1_sp_rs = (SP(Rs,1) -SP(Rs,4))./(SP(Rs,1) + SP(Rs,4)); %running
% modulation_indx2_sp_rs = (SP(Rs,2) -SP(Rs,3))./(SP(Rs,2) + SP(Rs,3)); %large pupil + sit
% modulation_indx1_sp_fs = (SP(fs,1) -SP(fs,4))./(SP(fs,1) + SP(fs,4)); %running
% modulation_indx2_sp_fs = (SP(fs,2) -SP(fs,3))./(SP(fs,2) + SP(fs,3)); %large pupil + sit

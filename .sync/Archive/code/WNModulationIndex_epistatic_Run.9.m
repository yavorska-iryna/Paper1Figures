    ...
    if data(cc).dir < 30
        ...
        if meanSpikeCount > 3 && CellsQualityStats.SNR(cc)>.5 && CellsQualityStats.uQ(cc)>10
            ...
            if data(cc).nrepsWNMon > nreps_check_running && data(cc).nrepsWNMoff > nreps_check_running %&& data1(cc).nrepsWNMon > nreps_check_running && data1(cc).nrepsWNMoff > nreps_check_running
                meanWN(cc,1)= median([nanmean(nanmean(data(cc).mWNon))]);% nanmean(nanmean(data(cc).mNSustained_on)) nanmean(nanmean(data(cc).mNOFF_on))]);
                meanWN(cc,2)= median([nanmean(nanmean(data(cc).mWNoff))]);% nanmean(nanmean(data(cc).mNSustained_off)) nanmean(nanmean(data(cc).mNOFF_off))]);
                meanON(cc,1) = median([nanmean(nanmean(data(cc).mNON_on))]);%-nanmean(nanmean(data(cc).mNson));
                meanON(cc,2) = median([nanmean(nanmean(data(cc).mNON_off))]);%-nanmean(nanmean(data(cc).mNsoff));
            ...
            if data(cc).nrepsSSMon > nreps_check_running && data(cc).nrepsSSMoff > nreps_check_running %&& data1(cc).nrepsSSMon > nreps_check_running && data1(cc).nrepsSSMoff > nreps_check_running
                SP(cc,1) = median(nanmean(data(cc).mSSon)); SP(cc,2) = median(nanmean(data(cc).mSSoff));
...
% MI_sound_sit(indx) = NaN;
% MI_sound_run(indx) = NaN;
% MI_sound_run_laser(indx) = NaN;
% MI_sound_sit_laser(indx) = NaN;
% MI_sound_predicted(indx) = NaN;

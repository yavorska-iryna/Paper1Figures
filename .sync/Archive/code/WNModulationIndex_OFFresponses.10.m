...
meanOFF = nan(length(data),2); % On response only (0 - 100 ms)
meanOFFL = nan(length(data),2);
...
for cc =1:length(data)
    if data(cc).dir < 47
        if data(cc).dir~=23 && data(cc).dir~=0 % ecluding putlier recordings
        try
        ...
        if meanSpikeCount > 2  && CellsQualityStats.SNR(cc)>.5 && CellsQualityStats.uQ(cc)>10
            ...
            if data(cc).nrepsWNMon > nreps_check_running && data(cc).nrepsWNMoff > nreps_check_running %&& data1(cc).nrepsWNMon > nreps_check_running && data1(cc).nrepsWNMoff > nreps_check_running
                meanOFF(cc,1) = nanmean(nanmean(data(cc).mNOFF_on));%-nanmean(nanmean(data(cc).mNson));
                meanOFF(cc,2) = nanmean(nanmean(data(cc).mNOFF_off));%-nanmean(nanmean(data(cc).mNsoff));
            ...
            if data1(cc).nrepsWNMon > nreps_check_running && data1(cc).nrepsWNMoff > nreps_check_running % && data(cc).nrepsWNMon > nreps_check_running && data(cc).nrepsWNMoff > nreps_check_running
                meanOFFL(cc,1) =nanmean([nanmean(nanmean(data1(cc).mNOFF_on))]);%-nanmean(nanmean(data1(cc).mNson));
                meanOFFL(cc,2) =nanmean([nanmean(nanmean(data1(cc).mNOFF_off))]);%-nanmean(nanmean(data1(cc).mNsoff));
...
plot(nanmean(MI_sound_sit), nanmean(MI_sound_run), 'ro')
plot(nanmedian(MI_sound_sit), nanmedian(MI_sound_run), 'ro')

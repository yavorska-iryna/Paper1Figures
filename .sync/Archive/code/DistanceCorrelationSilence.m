...
for d = 1:length(DIRS)
    try
        ...
        for c = 1:length(good_cells1)
            ...
            [N,x] = hist(st1, 0:binwidth:recording_duration);
            N=N/binwidth;
            N=smooth(N,10); N = N';
            N=N/max(N);
            data=[data; N];
            ...
            %DC
%             try
%                 NumCells(d,1)=size(ST1,2);
%                 DC_pupil(d,1) = distcorr(ST1, P1);
%                 DC_running(d,1) = distcorr(ST1, M1);
%                 
%             catch
%                 DC_pupil(d,1) = NaN;
%                 DC_running(d,1) = NaN;
...
%                 DC_running(d,2) = distcorr(ST2, M1);
%             catch
%                 DC_pupil(d,2) = NaN;
%                 DC_running(d,2) = NaN;
            end

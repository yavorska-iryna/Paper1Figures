...
for c = 1 : length(data)
    if evoked1(c) ==1
        ...
        num_reps_sit = size(FR_sit,1);
        num_reps_run = size(FR_run,1);
        
        if num_reps_sit > 6 && num_reps_run > 6
            indx = randi(num_reps_run, 1, num_reps_sit); % generate indices to match running trials
            cc = cc+1;
            MI_sit = FR_sit(indx) - SP_sit(indx)/FR_sit(indx) + SP_sit(indx);
            MI_run = FR_run(indx) - SP_run(indx)/FR_run(indx) + SP_run(indx);
        end
    end
end

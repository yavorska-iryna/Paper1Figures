...
meanON = nan(length(data),1);  meanONL = nan(length(data),1); 
evoked = zeros(length(data),4); 
...
for cc =1:length(data)
    if data(cc).dir<30
        ...
        if meanSpikeCount > 2 && CellsQualityStats.SNR(cc)>.5 && CellsQualityStats.uQ(cc)>10 
            ...
            Spont = [data(cc).mSSon; data(cc).mSSoff; data(cc).pSSon]; % spont trials in all states
            
            ON = [data(cc).mNON_on; data(cc).mNON_off; data(cc).pNON_on]; % data(cc).mNON_on; data(cc).mNON_off]; %ON = nanmean(ON);
            [rs, h, stats] = ranksum( ON(:), Spont(:));
            evoked(cc,1) = h;
            zstats(cc,1) = stats.zval;
            
            Sust = [data(cc).mNSustained_on; data(cc).mNSustained_off; data(cc).pNSustained_on]; 
            [rs, h, stats] = ranksum( Sust(:), Spont(:));
            evoked(cc,2) = h;
            zstats(cc,2) = stats.zval;
            
            OFF = [data(cc).mNOFF_on; data(cc).mNOFF_off; data(cc).pNOFF_on]; 
            ...
            if data(cc).nrepsWNMon > nreps_check_running && data(cc).nrepsWNMoff > nreps_check_running && data1(cc).nrepsWNMon > nreps_check_running && data1(cc).nrepsWNMoff > nreps_check_running
                ...
                meanON(cc,1) =nanmean([nanmean(nanmean(data(cc).mNON_on))]);
                meanON(cc,4) =nanmean([nanmean(nanmean(data(cc).mNON_off))]);
            ...
            if data(cc).dir~= 0 
                if data(cc).nrepsWNPon > nreps_check_pupil && data(cc).nrepsWNPoff > nreps_check_pupil && data1(cc).nrepsWNPon > nreps_check_pupil && data1(cc).nrepsWNPoff > nreps_check_pupil
                    ...
                    meanON(cc,2)= nanmean(nanmean(data(cc).pNON_on));%nanmean([nanmean(nanmean(data(cc).pNON_on)) nanmean(nanmean(data(cc).pNSustained_on)) nanmean(nanmean(data(cc).pNOFF_on))]);
                    meanON(cc,3)= nanmean(nanmean(data(cc).pNON_off));
                end
            end
...
% laser on trials
            if data1(cc).nrepsWNMon > nreps_check_running && data1(cc).nrepsWNMoff > nreps_check_running && data(cc).nrepsWNMon > nreps_check_running && data(cc).nrepsWNMoff > nreps_check_running
                ...
                meanONL(cc,1) =nanmean([nanmean(nanmean(data1(cc).mNON_on))]);
                meanONL(cc,4) =nanmean([nanmean(nanmean(data1(cc).mNON_off))]);
            ...
            if data(cc).dir~= 0 
                if data1(cc).nrepsWNPon > nreps_check_pupil && data1(cc).nrepsWNPoff > nreps_check_pupil && data(cc).nrepsWNPon > nreps_check_pupil && data(cc).nrepsWNPoff > nreps_check_pupil
                    ...
                    meanONL(cc,2) =nanmean([nanmean(nanmean(data1(cc).pNON_on))]);
                    meanONL(cc,3) =nanmean([nanmean(nanmean(data1(cc).pNON_off))]);
                end
            end
...
SP = [MeanmSSon MeanpSSon MeanpSSoff MeanmSSoff]; % silent sound laser off
SPL = [MeanmSSonL MeanpSSonL MeanpSSoffL MeanmSSoffL]; % silent sound laser on
...
modulation_indx1_rsL = (WN1L(rs1,1) -WN1L(rs1,4))./(WN1L(rs1,1) + WN1L(rs1,4)); %running
modulation_indx2_rsL = (WN1L(rs1,2) -WN1L(rs1,3))./(WN1L(rs1,2) + WN1L(rs1,3)); %large pupil + sit
modulation_indx1_fsL = (WN1L(fs1,1) -WN1L(fs1,4))./(WN1L(fs1,1) + WN1L(fs1,4)); %running
modulation_indx2_fsL = (WN1L(fs1,2) -WN1L(fs1,3))./(WN1L(fs1,2) + WN1L(fs1,3)); %large pupil + sit

modulation_indx1_sp_rsL = (SP1L(rs1,1) -SP1L(rs1,4))./(SP1L(rs1,1) + SP1L(rs1,4)); %running
modulation_indx2_sp_rsL = (SP1L(rs1,2) -SP1L(rs1,3))./(SP1L(rs1,2) + SP1L(rs1,3)); %large pupil + sit
modulation_indx1_sp_fsL = (SP1L(fs1,1) -SP1L(fs1,4))./(SP1L(fs1,1) + SP1L(fs1,4)); %running
modulation_indx2_sp_fsL = (SP1L(fs1,2) -SP1L(fs1,3))./(SP1L(fs1,2) + SP1L(fs1,3)); %large pupil + sit

% modulation index total
modulation_indx1 = (WN1(:,1) -WN1(:,4))./(WN1(:,1) + WN1(:,4)); %running
modulation_indx1_sp= (SP1(:,1) -SP1(:,4))./(SP1(:,1) + SP1(:,4));

EvokedWN = actWN1- actSP1;
...
set(gcf, 'Poition',  [17 558 1223 420])
%%
recs1= (recs(evoked2));
recs_rs= (recs1(rs1));
recs_fs=(recs1(fs1));

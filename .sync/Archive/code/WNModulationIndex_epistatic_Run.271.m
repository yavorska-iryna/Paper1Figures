...
% plot MI
figure; hold on
plot(MI_sound_sit ,MI_sound_run, 'ko')
...
figure; hold
plot(actual_diff, predicted_diff, 'ko')
xlabel('Running Effect (MI diff)')
ylabel('Laser Effect (MI diff)')
plot([-2 1], [0 0], 'k--')
plot([0 0], [-2 1], 'k--')
...
figure; hold on
plot(MI_sound_sit, MI_sound_sit_laser, 'ko')
plot([-1 1], [-1 1], 'r--')
plot([-1 1], [0 0], 'k--')
plot( [0 0], [-1 1],'k--')
...
figure; hold on;
plot(WN1(:,2), WN1L(:,2), 'ko')
maxFR = max(max([WN1(:,2), WN1L(:,2)]));
plot(nanmean(WN1(:,2)), nanmean(WN1L(:,2)), 'ro')
plot([0 maxFR], [0 maxFR], 'r-')
title(title_str)
xlabel('FR laser off'); ylabel('FR laer on')
pbaspect([1 1 1]); set(gcf, 'PaperPositionMode', 'auto');

figure; hold on;
plot(SP1(:,2), SP1L(:,2), 'ko')
...
figure; hold on;
plot(MI_sound_sit, MI_sound_sit_laser, 'k.')
...
ylabel('sound modulation index - run')
pbaspect([1 1 1]); set(gcf, 'PaperPositionMode', 'auto');
...
MI1_rsL = (WN1L(rs1,1) -WN1(rs1,1))./(WN1L(rs1,1) + WN1(rs1,1)); %running rs
MI1_fsL = (WN1L(fs1,1) -WN1(fs1,1))./(WN1L(fs1,1) + WN1(fs1,1)); %running fs

MI1_sp_rs = (SP1L(rs1,1) -SP1(rs1,1))./(SP1L(rs1,1) + SP1(rs1,1)); %running
MI1_sp_fs = (SP1L(fs1,1) -SP1(fs1,1))./(SP1L(fs1,1) + SP1(fs1,1)); %running

data = [modulation_indx1_rs modulation_indx2_rs modulation_indx2_rs];

[p,tbl] = anova2(data,recs);
% VIP effect in ech state

figure(210); hold on
bar([.8  1.8 ], [nanmean(MI1_sp_rs) nanmean(MI1_rsL)], 'BarWidth', .1)
bar([.9  1.9], [nanmean(MI1_sp_fs) nanmean(MI1_fsL)], 'BarWidth', .1)


errorbar([.8 .9 1.8 1.9], [nanmean(modulation_indx3_sp_rs) nanmean(modulation_indx3_sp_fs) nanmean(modulation_indx3_rs) nanmean(modulation_indx3_fs) ...

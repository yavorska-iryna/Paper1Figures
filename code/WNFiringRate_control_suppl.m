...
title(sprintf('Evoked, rho=%.4f,p=%d ', rho, p))
xlabel('run - VIP'); ylabel('VIP - run'); lsline
pbaspect([1 1 1]);

subplot(2,1,2);hold on
plot([0 0], [max(abs([SP2; SP2L]))*-1 max(abs([SP2; SP2L]))], 'k--')
plot([max(abs([SP2; SP2L]))*-1 max([SP2; SP2L])], [0 0], 'k--')
plot(SP_rs, SP_rsL, 'ko')
plot(SP_fs, SP_fsL, 'go')
ylim([max(abs([SP2; SP2L]))*-1 max(abs([SP2; SP2L]))])
xlim([max(abs([SP2; SP2L]))*-1 max(abs([SP2; SP2L]))])
xlabel('FR change (running laser off trials)'); ylabel('FR change (sitting laser on trials)'); lsline
[rho,p]=corr(SP2, SP2L, 'Type', 'Spearman', 'rows', 'pairwise');
title(sprintf('Spont,  rho=%.4f,p=%d ', rho, p))
pbaspect([1 1 1]);

figure; subplot(2,1,1);hold on
plot([0 0], [max(abs([WN2_plus; WN2_exp]))*-1 max(abs([WN2_plus; WN2_exp]))], 'k--')
plot([max(abs([WN2_plus; WN2_exp]))*-1 max([WN2_plus; WN2_exp])], [0 0], 'k--')
plot(WN_rs_plus, WN_exp_rs, 'ko'); 
plot(WN_fs_plus, WN_exp_fs, 'go');
ylim([max(abs([WN2_plus; WN2_exp]))*-1 max(abs([WN2_plus; WN2_exp]))])
xlim([max(abs([WN2_plus; WN2_exp]))*-1 max(abs([WN2_plus; WN2_exp]))])
xlabel('FR change (running laser on trials)'); ylabel('Expected FR change'); lsline
[rho,p]=corr(WN2_plus, WN2_exp, 'Type', 'Spearman', 'rows', 'pairwise');
title(sprintf('Evoked, rho=%.4f,p=%d ', rho, p))
pbaspect([1 1 1]);

subplot(2,1,2);hold on
plot([0 0], [max(abs([SP2_plus; SP2_exp]))*-1 max(abs([SP2_plus; SP2_exp]))], 'k--')
plot([max(abs([SP2_plus; SP2_exp]))*-1 max([SP2_plus; SP2_exp])], [0 0], 'k--')
plot(SP_rs_plus, SP_exp_rs, 'ko'); 
plot(SP_fs_plus, SP_exp_fs, 'go');
ylim([max(abs([SP2_plus; SP2_exp]))*-1 max(abs([SP2_plus; SP2_exp]))])
xlim([max(abs([SP2_plus; SP2_exp]))*-1 max(abs([SP2_plus; SP2_exp]))])
xlabel('FR change (running laser on trials)'); ylabel('Expected FR change'); lsline
